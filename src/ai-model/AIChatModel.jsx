...
export default function AIChatModel() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#21221ecd] via-[#245e7f] to-[#4c98a3] flex flex-col items-center justify-center p-4 gap-8">
      <h1 className="text-6xl sm:text-7xl
        bg-gradient-to-r from-[#e16767] via-[#e2f1f1] to-[#4add74]
        bg-clip-text text-transparent text-center text-shadow-lg/20">
        AI Chat With Models
      </h1>

      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className={`px-4 py-2 rounded-full text-sm
        ${aiReady ? "bg-[#149b2689] text-white font-semibold border border-[#149b2689] item items-center gap-3 text"
        ...
        <div className="flex items-center gap-2" hidden={!aiReady}>
          ...
          <select value={selectedModel} onChange={handleModelChange} disabled={!aiReady}
            className="bg-amber-600 border border-amber-400
          ...
          disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer font-semibold">
            {models.map((model) => (
              <option key={model.id} value={model.id} className="bg-amber-800 open:border-0">
      ...
      bg-gradient-to-br from-orange-950/90 to-amber-950
      backdrop:blur-md border border-orange-800/50 rounded-3xl p-6">
        <div className="flex items-center justify-center
        mb-4 p-2 bg-gradient-to-r from-orange-600/20 to-red-500/20
        rounded-xl border border-orange-300/30">
        ...
        border-gray-600 mb-6 p-4 bg-gradient-to-b from-gray-900/50 to-gray-800/50 rounded-2xl">
          {messages.length === 0 && (
...

          {
            messages.map((msg) => (
              <div key={msg.id} className={`
                ...
                ${
                  msg.isUser ? "bg-gradient-to-r from-orange-500 to-red-500 text-white ml-auto text-right"
          ...
          {
            isLoading && (
              <div className="p-3 m-2 rounded-2xl max-w-xs bg-gradient-to-r from-amber-600 to-red-600 text-white">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <input type="text" value={inputValue} onChange={(event) => setInputValue(event.target.value)}
            ...
            className="flex-1 px-4 py-3 bg-yellow-900/30 border border-amber-800 rounded-2xl
          ...
          <button onClick={sendMessage}
            ...
            className="px-6 py-3 bg-gradient-to-r from-orange-400 to-red-500 hover:opacity-80
